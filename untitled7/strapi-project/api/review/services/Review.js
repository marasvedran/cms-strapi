...
module.exports = {
  ...
  fetch: (params) => {
    ...
    return Review
      .findOne(_.pick(params, _.keys(Review.schema.paths)))
      .populate(populate);
  },
  ...
  add: async (values) => {
    ...
    const entry = await Review.create(data);

    // Create relational data and return the entry.
    return Review.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Review.associations.map(a => a.alias));
    const data = _.omit(values, Review.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Review.update(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Review.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Review.attributes).reduce((acc, curr) => {
      switch (Review.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
